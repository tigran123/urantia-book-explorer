...
if (isset($text)) {
   ...
   switch ($search_mode) {
      case 0:   //точный поиск
         ...
         break;
      case 1:   //все слова
         $text = preg_replace('/(?<=\s|^)[*+](?=\s|$)\s?/u', '', $text); //Убираем отдельно стоящие символы * и +. Их использовать можно только в составе слова
      ...
      case 2:   //любое слово
         $text = preg_replace('/(?<=\s|^)[*+](?=\s|$)\s?/u', '', $text); //Убираем отдельно стоящие символы * и +. Их использовать можно только в составе слова
         $pattern = '/(\\b' . str_replace([' ','*','+'],['\\b|\\b','\\w*','\\w+'], trim($text)) . '\\b)/u';   //Заменяем пробел на ИЛИ, * и + на спецстроку
         break;
   }
