...
if ($text) {
   ...
   $pattern = '/(' . preg_replace('/\b(\w+)\b/u', '(<em>)?[.,;"«!?(–-]?\s?$1\s?[.,;"»!?)–-]?\s?(<\\\\/em>)?', $text) . ')/u'; //здесь – и - это разные тире: первое длинное, второе короткое
   $pattern = preg_replace('/\[\.,;"«!\?\(–-\]\?\\\\s\?/u', '', $pattern, 1); //убираем первое вхождение знаков, чтобы не выделялись предшествующие пробелы, запятые и пр. символы...
