...
function output_line($line, $full_context) {
   ...
   if ($shortcontext != 0 && $full_context == 0) {
      ...
      if ($shortcontext == 1) {
         ...
         $enddots_    = "";
         $res_line    = "";
         //https://regex101.com/r/jbZ7vT/10/
         $cont_count  = preg_match_all('/([.?!]?[”)]?(?:<[^>]*?>)?)((?:[^.\n!?]|\d\.\d|\. ?[CDэ]\.|\.U)*?)(<mark>.*?<\/mark>)((?:[^.\n!?]|\d\.\d|\. ?[CDэ]\.|\.U)*(?:[.:!?](?!\d)|$)[^ \n]*)(.?)/u',$line,$shortline,PREG_SET_ORDER);
         //                              (____________1___________)(_________2___________________________)(________3________)(_______________________________4_____________________________)(5_)
         $last_count  = $cont_count-1;//              |                      |                                     |                                         |                                |
         for ($r = 0; $r < $cont_count; $r++) {//     |                      |                                     |                                         |                                |
            $start_symb = $shortline[$r][1];//________|                      |                                     |                                         |                                |
            $start_line = $shortline[$r][2];//_______________________________|                                     |                                         |                                |
            $mark_line  = $shortline[$r][3];//_____________________________________________________________________|                                         |                                |
            $end_line   = $shortline[$r][4];//_______________________________________________________________________________________________________________|                                |
            $finish_symb= $shortline[$r][5];//________________________________________________________________________________________________________________________________________________|
            $startdots  = " ... ";
            $enddots    = " ... ... ... ";
